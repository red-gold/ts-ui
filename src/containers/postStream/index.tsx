 ...
 */
const handleWidth = (value: number, posts: Map<string, any>) => {
    if (value > 1400) {
...
const creatColumn = (numberOfCoulmn: number, posts: Map<string, any>) => {
    const postList: any[] = [];
    let currentIndex = 0;
    const scoreArray = ArrayAPI.getZeroArray(numberOfCoulmn);
    posts.forEach((item: Map<string, any>) => {
...
export class PostStreamComponent extends Component<IPostStreamProps & WithTranslation, IPostStreamState> {
    ...
    writePostElement = () => {
        ...
        return (
            <div
                className={classNames('grid-cell animate-top', classes.gridCell)}
        ...
        (posts || []).forEach((items: any[], index) => {
            const column = (
                <div className={classes.postBox} key={`post-stream-column-${index}`}>
                    ...
                    {items.map((post, indexpost) => (
                        <Fragment key={`post-container-${post.get('id')}`}>
    ...
    render() {
        ...
        return (
            ...
            >
                ...
                {posts && !(posts.count() > 0) ? this.writePostElement() : ''}
                <div className={classes.container}>{posts && posts.count() > 0 && this.getpostList()}</div>
            </InfiniteScroll>
        );
    }
}
