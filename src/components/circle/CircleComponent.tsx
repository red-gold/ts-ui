 ...
 */
export class CircleComponent extends Component<ICircleComponentProps & WithTranslation, ICircleComponentState> {
    styles = {
        userListItem: {
     ...
     *
     */
    constructor(props: ICircleComponentProps & WithTranslation) {
        super(props);
    ...
    handleUpdateCircle = () => {
        ...
        const { updateCircle } = this.props;
        if (updateCircle && circleName && circleName.trim() !== '') {
            updateCircle({ name: circleName, id: this.props.id });
        }
    };
     ...
     */
    handleDeleteCircle = () => {
        const { deleteCircle, id } = this.props;
        if (deleteCircle) {
            deleteCircle(id);
        }
    };
    ...
    userList = () => {
        const { usersOfCircle, goTo } = this.props;
        const usersParsed: any = [];

        if (usersOfCircle && goTo) {
            ...
            <div>
                <div style={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between' }}>
                    ...
                    <div style={{ marginTop: '-9px' }}>
                        <Button color="primary" disabled={this.state.disabledSave} onClick={this.handleUpdateCircle}>
                            {' '}
                            {t('circle.saveButton')}{' '}
                        </Button>
                    </div>
        ...
        return (
            <div>
                ...
                <Dialog
                    ...
                    key={this.props.id}
                    open={this.props.openSetting || false}
...
const translateWrapper = withTranslation('translations')(CircleComponent);
