...
export const postReducer = (state = Map(new PostState() as any), action: IPostAction) => {
    ...
    switch (action.type) {
        ...
        case PostActionType.ADD_IMAGE_POST:
            return state.setIn(['entities', payload.post.get('id')], payload.post);
        ...
        case PostActionType.UPDATE_POST_COMMENTS:
            return updatePostComments(state, payload);
        case PostActionType.UPDATE_POST_VOTES:
        ...
        case PostActionType.ADD_LIST_POST:
            return state.mergeIn(['entities'], payload.entities).set('loaded', true);

        case PostActionType.ADD_LIST_STREAM_POST:
            return state.mergeIn(['stream', 'list'], payload.postIds).set('loaded', true);

        case PostActionType.ADD_LIST_SEARCH_POST:
            return addSearchPosts(state, action);
        ...
        case PostActionType.POST_RESET_SEARCH:
            return state.setIn(['search', 'hasMoreData'], true).setIn(['search', 'list'], Map({}));

        case PostActionType.ADD_LIST_POST_INSTAGRAM:
            return state.mergeDeepIn(['instagram'], payload.posts);
