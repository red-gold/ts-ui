...
export const imageGalleryReducer = (state = Map(new ImageGalleryState() as any), action: IImageGalleryAction) => {
    ...
    switch (action.type) {
        ...
        case ImageGalleryActionType.ADD_AVATAR_IMAGE_LIST:
            return state.mergeDeepIn(['avatar', payload.userId, 'list'], payload.imageIds).set('loaded', true);

        case ImageGalleryActionType.ADD_COVER_IMAGE_LIST:
            return state.mergeDeepIn(['cover', payload.userId, 'list'], payload.imageIds).set('loaded', true);

        case ImageGalleryActionType.HAS_MORE_DATA_ALBUM_IMAGE:
            return state.setIn(['album', payload.albumId, 'hasMoreData'], true);

        case ImageGalleryActionType.NO_MORE_DATA_ALBUM_IMAGE:
            return state.setIn(['album', payload.albumId, 'hasMoreData'], false);

        case ImageGalleryActionType.LAST_ALBUM_IMAGE_ID:
            return state.setIn(['album', payload.albumId, 'lastImageId'], payload.imageId);

        case ImageGalleryActionType.ADD_ALBUM_ID_LIST:
            return state.mergeDeepIn(['album-todo', 'entities'], payload.entities).set('loaded', true);
        ...
        case ImageGalleryActionType.SET_IMAGE_URL:
            return state.setIn(['imageURLList', payload.name], payload.url);

        case ImageGalleryActionType.ADD_VIDEO_GALLERY:
            return state.update('videos', (videoList: any) => videoList.push(payload));
