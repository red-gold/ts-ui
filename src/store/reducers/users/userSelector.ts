...
const selectFindPeoplePage = () => {
    return createSelector([getFindPeoplePage], (page) => page);
};
const selectLastAlbumPage = () => {
    return createSelector([getAlbumLastPageRequest], (page) => page);
};

const selectMoreAlbum = () => {
    return createSelector([hasMoreAlbum], (moreAlbum) => moreAlbum);
};

const selectMoreFindPeople = () => {
    return createSelector([hasMoreFindPeople], (hasMoreFindPeople) => hasMoreFindPeople);
};

const selectMoreSearchPeople = () => {
    return createSelector([hasMoreSearchPeople], (hasMoreSearchPeople) => hasMoreSearchPeople);
};

const selectAlbumPosts = () => {
    return createSelector([getAlbumPosts, getPosts], (albumPosts, posts) => {
...
const selectFindPeople = () => {
    return createSelector([getFindPeople, getUsers], (people, entities) => {
...
const selectSearchPeople = () => {
    return createSelector([getSearchPeople, getUsers], (people, entities) => {
        let mappedPeople: List<Map<string, any>> = List([]);
        people.forEach((exist, userId) => {
            if (exist) {
                mappedPeople = mappedPeople.push(entities.get(userId));
...
const selectUsers = () => {
    return createSelector([getUsers], (users) => users);
};

const selectUserProfileById = () => {
    return createSelector([getUserProfileById], (userProfile) => userProfile);
};
